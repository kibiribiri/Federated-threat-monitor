...
CA_CERT         = "/home/Manager/ca.crt"
WINDOW_SIZE     = 30      # number of observations for training baseline
Z_THRESHOLD     = 3.0     # alert if Z-score exceeds this
POLL_INTERVAL   = 5       # seconds between log reads
EVENT_WINDOW    = 10      # seconds to count events within
...
def on_connect(client, userdata, flags, rc):
    ...
    if rc == 0:
        ...
        connected = False
        print(f"[{timestamp()}] Failed to connect, return code: {rc}")

def on_disconnect(client, userdata, rc):
    global connected
    connected = False
    print(f"[{timestamp()}] Disconnected from broker (rc={rc}). Entering BUFFERING state.")
                ...
                if match:
                    try:
                        log_time = datetime.strptime(
                            f"{datetime.utcnow().year} {match.group(1)}",
                            "%Y %b %d %H:%M:%S"
                        )
    ...
    while True:
        try:
            ...
            if z > Z_THRESHOLD:
                ...
                # Z <= threshold — update rolling baseline
                observations.append(event_count)
